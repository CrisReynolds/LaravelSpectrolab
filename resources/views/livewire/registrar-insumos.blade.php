<div>
    <x-menu>
        ...
        <div class="text-sm tracking-wide text-gray-500 dark:text-white mt-4 sm:mt-0 sm:ml-4"> {{ session('success') }}</div>
            ...
            @endif
            <div class="my-4">
                <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" wire:click="create">Agregar Artículo</button>
            </div>
            @if($isOpen)
            ...
            <form wire:submit.prevent="{{ $insumoId ? 'update' : 'store' }}">
                        <div class="mb-4">
                            <label for="detalle" class="block text-gray-700 font-bold mb-2">Nombre del artículo:</label>
                            <input type="text" wire:model="detalle" id="detalle" class=" w-full mb-2 border border-gray-300 px-4 py-2 rounded">
                                @error('detalle')
                                    <div class="mb-4">
                            ...
                            <input type="text" wire:model="codigo" id="codigo" class="w-5/6 mb-2 border border-gray-300 px-4 py-2 rounded">
                            <label for="marca" class="w-20 text-gray-700 font-bold mb-2 ">Marca:</label>
                            <input type="text" wire:model="marca" id="marca" class=" w-72 mb-2 border border-gray-300 px-4 py-2 rounded">
                            ...
                            <select wire:model="unidad_id" id="unidad_id" class="w-72 border border-gray-300 px-4 py-2 rounded">
                                <option value="">Selecciona una unidad</option>
                                    @foreach ($unidades as $unidad)
                                    <option value="{{$unidad->id}}">{{$unidad->unidad_ref}}</option>
                                    @endforeach
                            </select>
                            <label for="stock_minimo" class=" w-28 text-gray-700 font-bold mb-2 ">Cantidad mínima:</label>
                            <input type="number" wire:model="stock_minimo" id="stock_minimo" class=" w-28 border mb-2 border-gray-300 px-4 py-2 rounde">
                            <label for="categoria_id" class="w-20 text-gray-700 font-bold mb-2">Categoria:</label>
                            <select wire:model="categoria_id" id="categoria_id" class="w-72 mb-2 border border-gray-300 px-4 py-2 rounded">
                            ...
                            <input type="checkbox" wire:model="es_narcotico" id="es_narcotico" value="1" class=" h-9 w-9 border mb-2 border-gray-300 px-4 py-2 rounded-md ">
                            <br>
                            <label for="proveedor_id" class="w-20 text-gray-700 font-bold mb-2">Proveedor:</label>
                            <select wire:model="proveedor_id" id="proveedor_id" class="w-2/3 border border-gray-300 px-4 py-2 rounded">
                ...
                <tbody>  
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="flex items-center px-6 py-4 space-x-3">
                            <button wire:click="edit({{ $insumo->id }})" class="btn bg-yellow-400 hover:bg-yellow-500 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center"> 
