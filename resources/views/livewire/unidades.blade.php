...
<x-menu>
    <div class="flex justify-center items-center">
        <h2 class="text-4xl font-bold dark:text-black">Unidades</h2>
    </div>
    <section>
        @if (session('success'))
    ...
    <div class="text-sm tracking-wide text-gray-500 dark:text-white mt-4 sm:mt-0 sm:ml-4"> {{ session('success') }}</div>
        ...
        <h2 class="text-2xl font-bold mb-4">{{ $unidadId ? 'Editar Unidad' : 'Crear Unidad' }}</h2>
        <form wire:submit.prevent="{{ $unidadId ? 'update' : 'store' }}">
                    <div class="mb-4">
                        <label for="unidad_ref" class="block text-gray-700 font-bold mb-2">Unidad:</label>
                        <input type="text" wire:model="unidad_ref" id="unidad_ref" class="w-full border border-gray-300 px-4 py-2 rounded">
                    </div>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <tbody>  
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="flex items-center px-6 py-4 space-x-3">
                        <button wire:click="edit({{ $unidad->id }})" class="btn bg-yellow-400 hover:bg-yellow-500 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center"> 
                            <i class="fa-solid fa-pen-to-square"></i>
                        </button>
                        <button onclick="return confirm('Esta seguro de eliminar este registro?') || event.stopImmediatePropagation()" wire:click="deleteUnidad({{$unidad->id}})" class="btn bg-red-700 hover:bg-red-800 text-white font-bold py-2 px-4 rounded inline-flex items-center">
        ...
        {{ $unidades->links() }} <!-- Aquí se muestra la paginación -->
    </div>
